...
method_name="dropout_by_each_head_each_token_for_txt"   # shuffle_by_CLS, shuffle_by_last_text, dropout_by_last_text_attn_for_txt, dropout_by_last_text_attn_for_all, dropout_by_each_head_each_token_for_txt, dropout_by_each_head_each_token_for_all
...
deepspeed --master_port=29502 --include=localhost:4,5,6,7 llava/train/train_mem.py \
    ...
    --run_name "train_${max_train_samples}-${method_name}_keep_${shuffle_trivial_vision_tokens_keep_percentage}-vision_${vision_token_attn}-llava-v1.5-7b-finetune" \
    --output_dir "./outputs/checkpoints/train_${max_train_samples}_mix/${method_name}_keep_${shuffle_trivial_vision_tokens_keep_percentage}-vision_${vision_token_attn}-llava-v1.5-7b-finetune" \
...
#bash scripts/my_scripts/dropout_attn.sh
